...
// --- DATA ---
const measurements = [
    { id: 'r27.5', inches: '27.5"', cm: '70' }, { id: 'r28', inches: '28"', cm: '71' }, { id: 'r28.5', inches: '28.5"', cm: '72' },
    { id: 'r30', inches: '30"', cm: '76' },   { id: 'r31', inches: '31"', cm: '79' }, { id: 'r32', inches: '32"', cm: '82' },
    { id: 'r33', inches: '33"', cm: '84' },   { id: 'r34', inches: '34"', cm: '86.5' }, { id: 'r35', inches: '35"', cm: '89' },
    { id: 'r36', inches: '36"', cm: '91.5' }, { id: 'r37', inches: '37"', cm: '94' },   { id: 'r38', inches: '38"', cm: '96.5' },
    { id: 'r39', inches: '39"', cm: '99' },   { id: 'r40', inches: '40"', cm: '101.5' }, { id: 'r41', inches: '41"', cm: '104' },
    { id: 'r41.5', inches: '41.5"', cm: '105.5' }, { id: 'r42', inches: '42"', cm: '106.5' }, { id: 'r42.5', inches: '42.5"', cm: '108' },
    { id: 'r43', inches: '43"', cm: '109' },   { id: 'r43.5', inches: '43.5"', cm: '110.5' }, { id: 'r44', inches: '44"', cm: '111.5' },
    { id: 'r45', inches: '45"', cm: '114.5' }, { id: 'r46', inches: '46"', cm: '117' },   { id: 'r47', inches: '47"', cm: '119.5' },
    { id: 'r48', inches: '48"', cm: '122' },  { id: 'r49', inches: '49"', cm: '124.5' }, { id: 'r50', inches: '50"', cm: '127' },
...
// --- DOM Elements Cache (Declare, assign in DOMContentLoaded) ---
...
function updateOrderInStorage(updatedOrder) { const h = loadOrders(); const i = h.findIndex(order => order.id === updatedOrder.id); if (i !== -1) { h[i] = { ...h[i], ...updatedOrder }; saveOrders(h); return true; } return false; }
...
function createQuantityStepper(initialValue = 0, max = MAX_REEL_QUANTITY, onUpdateCallback = null) {
    ...
    const handleUpdate = () => {
        const v = parseInt(valueSpan.textContent);
        updateHighlight(v);
        if (onUpdateCallback) { onUpdateCallback(); }
    };

    minusBtn.addEventListener('click', () => { let v = parseInt(valueSpan.textContent); if (v > 0) { v--; valueSpan.textContent = v; plusBtn.disabled = v >= max; minusBtn.disabled = v <= 0; handleUpdate(); } });
    plusBtn.addEventListener('click', () => { let v = parseInt(valueSpan.textContent); if (v < max) { v++; valueSpan.textContent = v; minusBtn.disabled = v <= 0; plusBtn.disabled = v >= max; handleUpdate(); } });
...
function addVariationSection(bfValue = null, gsmValue = null, shadeValue = null, itemsToPopulate = null) {
    ...
    const updateTotal = () => {
        let total = 0;
        const steppers = section.querySelectorAll('.quantity-stepper span');
        steppers.forEach(span => {
            total += parseInt(span.textContent) || 0;
        });
        const totalDisplay = section.querySelector('.variation-section-total-display');
        if (totalDisplay) {
    ...
    measurements.forEach(m => {
        ...
        const stepper = createQuantityStepper(initialQty, MAX_REEL_QUANTITY, updateTotal);
        item.appendChild(stepper); listContainer.appendChild(item);
    });

    const footer = document.createElement('div');
    footer.className = 'variation-section-footer';
    footer.innerHTML = `Total: <span class="variation-section-total-display">0</span> Reels`;

    section.appendChild(listContainer);
    section.appendChild(footer);
    if(reelsVariationSectionsContainer) reelsVariationSectionsContainer.appendChild(section);
...
function displayOrderHistory() {
    ...
    if (h.length === 0) { historyListEl.innerHTML = '<li class="card"><p>No past orders found.</p></li>'; return; }
    h.forEach(o => {
        const li = document.createElement('li');
        const mainContent = document.createElement('div'); mainContent.className = 'history-item-main-content'; mainContent.onclick = () => showHistoryDetails(o.id); mainContent.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') showHistoryDetails(o.id); }; mainContent.setAttribute('role', 'button'); mainContent.setAttribute('tabindex', '0');
        const d = new Date(o.timestamp); const dS = d.toLocaleDateString('en-CA'); const tS = d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit'});
        ...
        let tI = 0; let iL = 'items';
        if (o.items?.length) { tI = o.items.reduce((s, i) => s + (parseInt(i.quantity) || 0), 0); if (o.type === 'Reels') iL = tI === 1 ? 'reel' : 'reels'; else iL = tI === 1 ? 'unit' : 'units'; }
        ...
        const editBtn = document.createElement('button'); editBtn.className = 'action-button secondary history-edit-btn'; editBtn.innerHTML = '<i class="fa-solid fa-pencil"></i> Edit'; editBtn.onclick = (e) => { e.stopPropagation(); editOrderFromHistory(o.id); };
        const deleteBtn = document.createElement('button'); deleteBtn.className = 'action-button history-delete-btn'; deleteBtn.innerHTML = '<i class="fa-solid fa-trash-can"></i> Delete'; deleteBtn.onclick = (e) => { e.stopPropagation(); confirmDeleteOrder(o.id); };
...
function formatOrderSummary(orderData) {
    ...
    let s = `Mill: ${orderData.mill||"N/A"}\nDate: ${dS}\n--- ${orderData.type.toUpperCase()} ORDER ---\n==========================\n\n`;
    let grandTotal = 0;

    if(orderData.type==='Reels' && orderData.items?.length){
        const g = orderData.items.reduce((a,i)=>{
            const k = `BF:${i.bf||"N/A"} | GSM:${i.gsm||"N/A"} | Shade:${i.shade||"N/A"}`;
            if(!a[k])a[k]=[];
            a[k].push({m:i.measurementTextWithUnit,q:i.quantity});
            grandTotal += parseInt(i.quantity) || 0;
            return a;
        },{});
        ...
        for(const k in g){
            ...
            g[k].forEach(i=>{
                const quantity = parseInt(i.q) || 0;
                s+=`${i.m}: ${quantity} ${quantity===1?'reel':'reels'}\n`;
                variationTotal += quantity;
            });
            s+=`*TOTAL: ${variationTotal} reels*\n`; // Add subtotal
            s+=`\n`;
        }
    } else if (orderData.items) {
        orderData.items.forEach(i=>{
            const quantity = parseInt(i.quantity) || 0;
            s+=`${i.name||"Item"}: ${quantity} ${quantity===1?'unit':'units'}\n`;
            grandTotal += quantity;
        });
    }

    s+=`==========================\n`;
    s+=`*TOTAL REELS: ${grandTotal}*`;
    return s;
}
...
function showHistoryDetails(orderId) { /* ... same as your last working version ... */ }
function formatHistoryReceiptSummary(orderData) { /* ... same as your last working version ... */ }
function shareHistoryReceiptSummary() { /* ... same as your last working version ... */ }
function acknowledgeOrder() { /* ... same as your last working version ... */ }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    reelsVariationSectionsContainer = document.getElementById('reels-variation-sections-container'); paperMillSelect = document.getElementById('paper-mill'); selectBf = document.getElementById('select-bf'); selectGsm = document.getElementById('select-gsm'); selectShade = document.getElementById('select-shade'); orderModal = document.getElementById('order-modal'); orderSummaryTextEl = document.getElementById('order-summary-text'); historyListEl = document.getElementById('history-list'); historyDetailsModal = document.getElementById('history-details-modal'); historyItemDetailsList = document.getElementById('history-item-details-list'); historyOrderIdInput = document.getElementById('history-order-id'); historyReceivedDateInput = document.getElementById('received-date'); historyModalDate = document.getElementById('history-modal-date'); historyModalMill = document.getElementById('history-modal-mill'); updateButton = document.getElementById('check-update-btn'); updateStatusMsg = document.getElementById('update-status-msg'); editingOrderIdInput = document.getElementById('editing-order-id'); reelsPageTitle = document.getElementById('reels-page-title'); generateReelsOrderBtnElement = document.getElementById('generate-reels-order-btn'); landingPageElement = document.getElementById('landing-page');
    ...
    if (landingPageElement) {
        landingPageElement.addEventListener('click', function(event) {
            ...
            if (event.target.closest('.update-section')) { return; }
            const pageId = item?.dataset?.page;
            if (pageId) { showPage(pageId); }
            else if (item) { alert('This feature is coming soon!'); }
        });
    }
    if (generateReelsOrderBtnElement) { generateReelsOrderBtnElement.addEventListener('click', () => generateOrder('Reels')); }
    document.addEventListener('click', function(event) { if (event.target.classList.contains('close') && event.target.closest('.modal')) { const modalIdToClose = event.target.dataset.modalId || event.target.closest('.modal').id; if (modalIdToClose) closeModal(modalIdToClose); } });
    window.addEventListener('click', function(event) { if (event.target.classList.contains('modal')) closeModal(event.target.id); });
    if (updateButton) { updateButton.addEventListener('click', () => { if (!navigator.serviceWorker?.controller) { if(updateStatusMsg) { updateStatusMsg.textContent = "Cannot check updates now (SW not ready)."; updateStatusMsg.className = 'update-status error'; } console.warn("Service worker controller not available."); return; } updateButton.disabled = true; if(updateStatusMsg) { updateStatusMsg.textContent = "Checking for updates..."; updateStatusMsg.className = 'update-status'; } updateButton.querySelector('i')?.classList.add('fa-spin'); navigator.serviceWorker.controller.postMessage({ action: 'checkForUpdate' }); }); }
    if ('serviceWorker' in navigator) { navigator.serviceWorker.addEventListener('message', event => { if (event.data?.status === 'updateCheckComplete') { if(updateButton) { updateButton.disabled = false; updateButton.querySelector('i')?.classList.remove('fa-spin'); } if(updateStatusMsg){ if (event.data.success) { updateStatusMsg.textContent = "Update check complete. RELOAD page."; updateStatusMsg.className = 'update-status success'; } else { updateStatusMsg.textContent = `Update check failed: ${event.data.error || 'Unknown'}`; updateStatusMsg.className = 'update-status error'; } } } }); }
    if ('serviceWorker' in navigator) { navigator.serviceWorker.register('./service-worker.js').then(reg => console.log('SW registered:', reg.scope)).catch(err => { console.error('SW registration failed:', err); if(updateStatusMsg) { updateStatusMsg.textContent = "Cannot check updates (SW error)."; updateStatusMsg.className = 'update-status error'; } if(updateButton) updateButton.disabled = true; }); } else { console.warn('Service Worker not supported.'); if(updateStatusMsg) { updateStatusMsg.textContent = "Cannot check updates (SW not supported)."; updateStatusMsg.className = 'update-status error'; } if(updateButton) updateButton.disabled = true; }
